...
func TestPool(t *testing.T) {
	...
	# HELP spectrum_pool_real_bytes Real capacity bytes in pool
	# TYPE spectrum_pool_real_bytes gauge
	spectrum_pool_real_bytes{id="0",name="Pool0"} 6.08317692968e+11
...
func TestIOgrpsDetail(t *testing.T) {
	c := newFakeClient()
	c.prepare("rest/lsiogrp/0", "testdata/lsiogrp_detail.jsonnet")
	r := prometheus.NewPedanticRegistry()
	if !probeIOgrpsDetail(c, r, []string{"0"}) {
    ...
    # HELP spectrum_iogrp_mem_flash_total Total memory allocated to FlashCopy
    # TYPE spectrum_iogrp_mem_flash_total gauge
    spectrum_iogrp_mem_flash_total{id="0",name="io_grp0"} 2.147169075e+09
    # HELP spectrum_iogrp_mem_raid_free Unused memory allocated to RAID
    # TYPE spectrum_iogrp_mem_raid_free gauge
    spectrum_iogrp_mem_raid_free{id="0",name="io_grp0"} 7.3505177e+07
    # HELP spectrum_iogrp_mem_raid_total Total memory allocated to RAID
    # TYPE spectrum_iogrp_mem_raid_total gauge
    spectrum_iogrp_mem_raid_total{id="0",name="io_grp0"} 1.572864e+08
	`
...
func TestIOgrps(t *testing.T) {
	c := newFakeClient()
	c.prepare("rest/lsiogrp", "testdata/lsiogrp.jsonnet")
	r := prometheus.NewPedanticRegistry()
	iogrps, err := getIOgrps(c, r)
	if err != nil {
		t.Errorf("probeIOgrps() returned non-success")
	}
	if len(iogrps) != 2 {
		t.Fatalf("getIOgrps() returned %d results.  Expected 2.", len(iogrps))
	}
	if iogrps[0] != "0" || iogrps[1] != "1" {
		t.Fatalf("getIOgrps() returned unexpected results (%s, %s)", iogrps[0], iogrps[1])
	}
}
